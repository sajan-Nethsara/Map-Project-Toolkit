...
def get_label_id(input_string,group_id):
    ...
    if len(split_string) == 3 :
        ...
        split_string[-1] = "label"
        split_string.insert(1,group_id)
        modified_string = "-".join(split_string)
        return modified_string


def get_group_id(input_string,group_id):
    split_string = input_string.split("-")
    if len(split_string) == 3 :
...
def get_group_id_converted_array(sheet):
    map_id = get_excel_array(sheet, 3)
    converted = convert_string_array(map_id)
    return converted
...
def handel_suites(element):
    ...
    for suite in suites_text_array:
        ...
        name = f"{nik_name}-{suite_text_converted}-label"
        suite["id"] = name
    ...
    if suite_path_element == None:
        ...
        suite_path_element["id"] = f"{nik_name}-section"
    print(Fore.BLUE + f"{nik_name} Group renamed successfully")


def manipulate_svg(map_text_list, map_id_list,group_list,svg_path):
    # return soup
    error_count = 0
    try:
    ...
    for element in all_group_elements:
        ...
        if element_id == "lawn-group" :
            ...
            group_id = group_list[search_index]
            group_name = get_group_id(map_id,group_id)
            section_name = get_section_id(map_id)
            label_name = get_label_id(map_id,group_id)
...
def main(excel_path,svg_path,new_svg):
    try:
        ...
        group_list = get_group_id_converted_array(sheet)
    except:
        print(Back.RED + "Error with loading EXCEL file")
        print(Style.RESET_ALL + '---')
    new_soup = manipulate_svg(map_text_list, map_id_list,group_list,svg_path)
    modified_svg_content = str(new_soup)
    with open(new_svg, "w") as output_file:
        output_file.write(modified_svg_content)
        print(f"{new_svg} file created")
...
def before_main():
    ...
    if svg_path == '' :
        print(Fore.RED , ".svg file not found in current folder")
        print(Style.RESET_ALL + '')
    if excel_path == '':
        print(Fore.RED , ".xlsx file not found in current folder")
        print(Style.RESET_ALL + '')
    if svg_path != '' and excel_path != '' :
        print("targeted .svg file")
        print(Fore.BLUE , f"--> {svg_path}")
        print(Style.RESET_ALL + '')
        print(f"targeted .xlsx file")
        print(Fore.BLUE , f"--> {excel_path}")
        print(Style.RESET_ALL + '')
        start = input("Press Enter to continue...")
        new_svg = f"renamed-{svg_path}"
        main(excel_path,svg_path,new_svg)
